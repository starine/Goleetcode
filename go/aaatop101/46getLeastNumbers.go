...
//方法一： 利用快排思想，通过partition找到索引为 K-1 的数，那么它左边的数就是比它小的另外 K-1 个数！不需要全部排序，最优解。
//时间复杂度： O(n)
...
// 注意：Push,Pop 实现了标准库 `container/heap` 的接口，不仅要改变元素值，还要改变数组长度，所以receiver用指针
...
func getLeastKNumbers(nums []int, k int) []int {
	h := &IMaxHeap{}
	heap.Init(h)
